"""Build the Qdrant index: embed all chunks and upload to 'regbrain' collection."""
...
UPLOAD_BATCH = 20        # points per Qdrant upsert (small to avoid cloud timeouts)
PROGRESS_EVERY = 100
MAX_RETRIES = 3
...
def main():
    ...
    print(f"\nUploading {len(points)} points (batch_size={UPLOAD_BATCH}) ...")
    t0 = time.time()
    uploaded = 0
    ...
    for batch_start in range(0, len(points), UPLOAD_BATCH):
        ...
        uploaded += len(batch)
        if uploaded % PROGRESS_EVERY == 0 or uploaded == len(points):
